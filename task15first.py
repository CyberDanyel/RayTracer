...
plano_convex_lens = SphericalRefraction(105, -0.02, 1.5168, 1)

plane = SphericalRefraction(100, 0, 1, 1.5168)

output = OutputPlane(250)

optical_element = OpticalElement([plane, plano_convex_lens, output])

output = OutputPlane(ParaxialFocusFinder([plane, plano_convex_lens]).find())
...
plt.savefig('T-15 Plane Then Convex', dpi = 800)
...
ax.axis('equal')
plt.grid()
plt.savefig('T-15 Spot Diagram Plane Then Convex', dpi = 800)
